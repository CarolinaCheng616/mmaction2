...
def statistic():
    ...
    train_meta, val_meta, pgm_proposal, iou = (
        args.train,
        args.val,
        args.pgm_proposal,
        args.iou,
    ...
    print(
        f"train positive per video: {train_pos / train_num}, "
        f"ratio: {train_pos / (train_pos + train_neg)}"
    )
    print(
        f"train negative per video: {train_neg / train_num}, "
        f"ratio: {train_neg / (train_pos + train_neg)}"
    )
    print(
        f"val positive per video: {val_pos / val_num}, "
        f"ratio: {val_pos / (val_pos + val_neg)}"
    )
    print(
        f"val negative per video: {val_neg / val_num}, "
...
def _multi_compute_iou_for_results(videos, results, meta, result_dict):
    for video in videos:
        ...
        for idx in range(len(result)):
            ...
            iou = np.max(
                temporal_iou(segment[0], segment[1], references[:, 0], references[:, 1])
            )
            dic = dict(
                score=result[idx]["score"], segment=result[idx]["segment"], iou=iou
            )
...
def compute_iou_for_results(result_file, meta_file, new_file):
    ...
    for i in range(threads):
        proc = Process(
            ...
            args=(
                videos[i * videos_per_thread : (i + 1) * videos_per_thread],
                results,
                meta,
                result_dict,
            ),
