...
def generate_tag_proposals_offset(
    ...
    for video_index in video_list:
        ...
        new_iou_list = np.array(new_iou_list).reshape(-1, 1)
        new_ioa_list = np.array(new_ioa_list).reshape(-1, 1)
        tmin_off_list = np.array(tmin_off_list).reshape(-1, 1)
        tmax_off_list = np.array(tmax_off_list).reshape(-1, 1)
        new_props = np.concatenate((new_props, new_iou_list), axis=1)
        new_props = np.concatenate((new_props, new_ioa_list), axis=1)
        new_props = np.concatenate((new_props, tmin_off_list), axis=1)
        new_props = np.concatenate((new_props, tmax_off_list), axis=1)
        proposal_dict[video_name] = new_props
        # tmin, tmax, action_score, iou, iop, tmin_offset, tmax_offset
        if result_dict is not None:
            result_dict[video_name] = new_props
    return proposal_dict
...
def _proposals_hard_nms(proposals, threshold, top_k, score_idx):
    """Hard NMS for tag temporal proposals.

    Args:
        proposals (np.ndarray): Proposals generated by network.
        threshold (float): iou threshold for hard nms.
        top_k (int): Top k values to be considered.
        score_idx (int): index to sort proposals.
...
def _proposals_hard_nms_de_duplicate(proposals, threshold, top_k, score_idx):
    """Hard NMS for tag temporal proposals.
...
def _proposals_post_processing(result, video_info, score_idx, soft_nms_alpha,
    ...
    Args:
        ...
        score_idx (int): index of proposal file to do soft nms.
...
def _proposals_post_processing_hard_nms(result, video_info, score_idx, threshold, top_k):
    """sort proposals by score_idx and soft nms, get top k proposals.
    ...
    """
    if len(result) > 1:
        result = _proposals_hard_nms(result, threshold, top_k, score_idx)
    result = result[result[:, score_idx].argsort()[::-1]][:top_k]
    # tmin, tmax, score, iou, iop(, tmin_offset, tmax_offset)
    video_duration = float(video_info['duration_second'])
    proposal_list = []
...
def _proposals_post_processing_hard_nms_de_duplicate(result, video_info, score_idx, threshold, top_k):
    """sort proposals by score_idx and soft nms, get top k proposals.
...
def _multithread_nms_and_dump_results(video_infos, pgm_proposals_dir,
    ...
    for vinfo in video_infos:
        ...
        proposal_list, post_proposal = _proposals_post_processing_hard_nms_de_duplicate(
...
def _generate_tag_original_feature(video_list,
    ...
    Args:
        ...
        top_k (int): Number of proposals to be considered. Default: -1
        bsp_boundary_ratio (float): Ratio for proposal boundary
            (start/end). Default: 0.2.
        num_sample_start (int): Num of samples for actionness in
            start region. Default: 8.
            ...
            y_new_start = [
                np.mean(
                    y_new_start_action[i * num_sample_interp:(i + 1) *
                                       num_sample_interp + 1],
                    axis=0) for i in range(num_sample_start)
            ]
...
if __name__ == '__main__':
    ...
    nms_proposal_dir = 'work_dirs/tag_pgm_snippet_offset_clipped_iou_nms_pos_neg/pgm_proposals/'
    nms_features_dir = 'work_dirs/tag_pgm_snippet_offset_clipped_iou_nms_pos_neg/pgm_features/'
    ann_file = 'data/TruNet/train_meta.json'
    # out = 'work_dirs/tag_pgm_snippet_clipped/test_iou_hard_nms_results.json'
    out = 'work_dirs/tag_pgm_snippet_offset_clipped_iou_nms_pos_neg/val_clipped_de_duplicate_results.json'
    iou_nms = True
