...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if self.id < self.depth_3d:
            ...
            self.conv_normal = nn.Conv2d(planes,
                                         planes,
                                         kernel_size=3,
                                         stride=1,
                                         padding=1,
        ...
        if n_s < self.depth_3d:
            self.conv4 = nn.Conv3d(planes,
                                   planes * 4,
                                   kernel_size=1,
                                   bias=False)
            self.bn4 = nn.BatchNorm3d(planes * 4)
        else:
            self.conv4 = nn.Conv2d(planes,
                                   planes * 4,
                                   kernel_size=1,
                                   bias=False)
...
class ResNetP3D1(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1_custom = nn.Conv3d(self.input_channel,
                                      64,
                                      kernel_size=(1, 7, 7),
                                      stride=(1, 2, 2),
                                      padding=(0, 3, 3),
        ...
        self.depth_3d = sum(
            layers[:3])  # C3D layers are only (res2,res3,res4),  res5 is C2D

        self.bn1 = nn.BatchNorm3d(64)  # bn1 is followed by conv1
        self.cnt = 0
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=(2, 3, 3),
                                    stride=2,
                                    padding=(0, 1,
                                             1))  # pooling layer for conv1.
        self.maxpool_2 = nn.MaxPool3d(
            kernel_size=(2, 1, 1), padding=0,
            stride=(2, 1, 1))  # pooling layer for res2, 3, 4.

        self.layer1 = self._make_layer(Bottleneck, 64, layers[0], shortcut_type)
    ...
    def scale_size(self):
        return self.input_size[
            2] * 256 // 160  # asume that raw images are resized (340,256).
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1):
        ...
        if self.cnt < self.depth_3d:
            ...
            if stride != 1 or self.inplanes != planes * block.expansion:
                # res2, res3, res4  and  the first conv layer of res1
                if shortcut_type == 'A':
        ...
        if isinstance(m, torch.nn.Conv3d) or isinstance(m, torch.nn.Conv2d):
            ...
            if len(list(m.parameters())) > 0:
                raise ValueError('New atomic module type: {}. '
                                 'Need to give it a learning policy'.format(
                                     type(m)))
    ...
    return [
        {
            ...
            'lr_mult': 5 if modality == 'Flow' else 1,
            'decay_mult': 1,
            'name': 'first_conv_weight'
        },
        {
            'params': first_conv_bias,
            'lr_mult': 10 if modality == 'Flow' else 2,
            'decay_mult': 0,
