...
class VideoAudioTextMatcherE2E(nn.Module):
    ...
    def __init__(
        ...
        self.img_audio_mlp = nn.Sequential(
            nn.Linear(img_feat_dim + audio_feat_dim, self.feature_dim * 2),
    ...
    def forward_train(self, imgs, audios, texts_item):
        ...
        v_a_feat = nn.functional.normalize(
            self.img_audio_mlp(torch.cat((v_feat, a_feat), dim=1)), dim=1
        )
    ...
    def forward_test(self, imgs, audios, texts_item):
        ...
        return zip(
            v_a_feat.cpu().numpy(), t_feat.view(N, -1, t_feat.shape[1]).cpu().numpy()
        )
