...
from .ff_lstm import FFLSTM


@LOCALIZERS.register_module()
class fcTEM(BaseLocalizer):
    ...
    def _forward(self, x):
        ...
        """
        # x.shape [batch, feature, temporal]
        x = x.permute(0, 2, 1)
        # x.shape [batch, temporal, feature]
        x = F.relu(self.fc(x))
        # x.shape [batch, temporal, resized_feature]
        x = x.permute(0, 2, 1)
        # x.shape [batch, resized_feature, temporal]
...
@LOCALIZERS.register_module()
class ClassifyBNPEM(BaseLocalizer):
...
# @LOCALIZERS.register_module()
# class ClassifyBNPEMRegLSTM(BaseLocalizer):
...
#         # transposed to L, N, C(32, batch, 4096)
#         x = self.ff_lstm(x)
#         # L, N, C1(32, batch, channel)
#         x = torch.transpose(x, 0, 1)
...
@LOCALIZERS.register_module()
class OriFeatBNPEM(BaseLocalizer):
    """Classify proposal into binary category: background or foreground using
    original features.
