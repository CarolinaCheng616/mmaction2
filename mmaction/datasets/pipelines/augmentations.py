...
class RandomScale:
    ...
    def __init__(self, scales, mode="range", **kwargs):
        ...
        if self.mode not in ["range", "value"]:
            raise ValueError(
                f"mode should be 'range' or 'value', " f"but got {self.mode}"
            )
        self.scales = scales
        self.kwargs = kwargs
    ...
    def __repr__(self):
        repr_str = (
            f"{self.__class__.__name__}(" f"scales={self.scales}, mode={self.mode})"
        )
        return repr_str
    ...
    def __call__(self, results):
        ...
        if proposals is not None:
            assert proposals.shape[1] == 4, (
                "proposals shape should be in " f"(n, 4), but got {proposals.shape}"
            )
...
class RandomResizedCrop:
    ...
    def __call__(self, results):
        ...
        if not self.lazy:
            results["imgs"] = [img[top:bottom, left:right] for img in results["imgs"]]
        else:
            lazyop = results["lazy"]
            if lazyop["flip"]:
                raise NotImplementedError("Put Flip at last for now")
...
class ColorJitter:
    ...
    def __init__(
        self, color_space_aug=False, alpha_std=0.1, eig_val=None, eig_vec=None
    ):
        if eig_val is None:
    ...
    def hue(img, alpha):
        ...
        if np.random.rand() > 0.5:
            ...
            bt = np.array(
                [[1.0, 0.0, 0.0], [0.0, u, -w], [0.0, w, u]], dtype=np.float32
            )
            tyiq = np.array(
                [
                    [0.299, 0.587, 0.114],
    ...
    def __call__(self, results):
        ...
        rgb = np.array(np.dot(self.eig_vec * alpha, self.eig_val), dtype=np.float32)
        rgb = rgb[None, None, ...]
...
class ThreeCrop:
    ...
    def __call__(self, results):
        ...
        if crop_h == img_h:
            ...
            offsets = [(0, 0), (2 * w_step, 0), (w_step, 0)]  # left  # right  # middle
        elif crop_w == img_w:
            h_step = (img_h - crop_h) // 2
            offsets = [(0, 0), (0, 2 * h_step), (0, h_step)]  # top  # down  # middle
...
class MultiGroupCrop:
    ...
    def __init__(self, crop_size, groups):
        ...
        if not mmcv.is_tuple_of(self.crop_size, int):
            raise TypeError(
                "Crop size must be int or tuple of int, " f"but got {type(crop_size)}"
            )
...
class RandomColorJitter:
    ...
    def __init__(
        self, color_space_aug=False, alpha_std=0.1, eig_val=None, eig_vec=None, p=0.5
    ):
        if eig_val is None:
            # note that the data range should be [0, 255]
            self.eig_val = np.array([55.46, 4.794, 1.148], dtype=np.float32)
