...
def process_checkpoint(in_file, out_file, prefix, replace):
    ...
    for key in checkpoint:
        if prefix in key:
            backbone[key.replace(prefix, replace)] = checkpoint[key]
...
def main():
    args = parse_args()
    process_checkpoint(args.in_file, args.out_file, args.prefix, args.replace)
